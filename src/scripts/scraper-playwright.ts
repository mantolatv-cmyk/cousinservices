...
async function main() {
  console.log('╔══════════════════════════════════════════════════════════╗');
  console.log('║   CousinServices — Playwright Scraper v1.0             ║');
  console.log('║   Zukerman + Mega Leilões → leiloes.json               ║');
  console.log('╚══════════════════════════════════════════════════════════╝');
  ...
  // Scrape Zukerman
  const pageZuk = await context.newPage();
  const zukItems = await scrapeZukerman(pageZuk);
  allItems.push(...zukItems);
  await pageZuk.close();

  // Scrape Mega Leilões
  const pageMega = await context.newPage();
  const megaItems = await scrapeMegaLeiloes(pageMega);
  allItems.push(...megaItems);
  await pageMega.close();
  ...
  fs.writeFileSync(outputPath, JSON.stringify({
    ...
    sources: {
      zukerman: zukItems.length,
      megaLeiloes: megaItems.length,
    },
    items: allItems,
  }, null, 2), 'utf-8');
  ...
  console.log(`  Zukerman:     ${zukItems.length} terrenos`);
  console.log(`  Mega Leilões: ${megaItems.length} terrenos`);
